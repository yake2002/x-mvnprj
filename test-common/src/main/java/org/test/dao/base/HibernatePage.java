...
//    }
     ...
     *
     * @param resultHql Hibernate的Query对象
     * @param total       统计总数
     ...
     */
    public HibernatePage(String resultHql, Session session, boolean cacheable, long total, int pageNumber, int pageSize, Object... values) {
        Query resultQuery=session.createQuery(resultHql);
        this.pageNumber = pageNumber;
        this.pageSize = pageSize;
        int maxResSize = this.pageSize;
        try {
            String countHql = "select count(*) " + removeSelect(removeOrders(resultQuery.getQueryString()));
            Query countQuery = session.createQuery(countHql);
            if (cacheable) {
            ...
            this.totalElements = count < total ? count : total;
            if (Integer.MAX_VALUE == this.pageNumber || this.pageNumber >= getLastPageNumber())   //last page
            {
                this.pageNumber = getLastPageNumber();
                maxResSize = (int)(this.totalElements - (this.pageNumber - 1) * this.pageSize);
            }
            if (this.pageNumber < 1) {
                this.pageNumber = 1;
            }

            elements = resultQuery.setFirstResult((this.pageNumber - 1) * this.pageSize).setMaxResults(maxResSize).list();
        } catch (HibernateException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public HibernatePage(Query resultQuery, long total, int pageNumber, int pageSize) {
        ...
        try {
            ...
            {
                this.pageNumber = getLastPageNumber();
                maxResSize = (int)(total - (this.pageNumber - 1) * this.pageSize);
            }
            if (this.pageNumber < 1) {
                this.pageNumber = 1;
            }
